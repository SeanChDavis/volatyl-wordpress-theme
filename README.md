...
The theme operates in one of two structural modes, set globally in the Customizer under **Design**:
...
Volatyl uses a mathematically-generated color palette built on the [OKLCH color space](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value/oklch). Instead of managing individual color values, you control three inputs and the theme handles the rest.

### Three Controls, One Palette

- **Primary Hue** (0–360°) — The base hue that anchors the entire palette. Every color in the theme derives from this value.
- **Palette Vibrancy** (0–100%) — How vivid the palette is. At 0, all brand colors are near-gray. As the value increases, buttons, links, headings, and accent colors gain chroma.
- **Background Tint** (0–100%) — How much the primary hue influences dark backgrounds and body text. At 0, dark surfaces and text are neutral. As the value increases, they take on a subtle tint of the primary hue. Intentionally capped at a low range to keep dark sections readable at all settings.

### Color Scheme Types

The scheme type determines how the action and accent colors relate to the primary hue. The **action** color is always anchored to the primary hue — the scheme only affects the accent families.

| Scheme | Description |
|---|---|
| **Monochromatic** | All colors share the primary hue |
...
The full palette is available as CSS custom properties on both the front end and in the block editor.
...
When you apply a background color to a block from the theme palette, text colors adapt automatically:

- **Dark and darker variants** (`has-action-dark-background-color`, `has-accent-1-dark-background-color`, etc.) — full dark context: text goes near-white, buttons, forms, and tables all adapt for readability.
- **Base color variants** (`has-action-background-color`, `has-accent-1-background-color`, etc.) — text, headings, and links go white. Buttons and forms are left as-is.
- **Light, lighter, and tint variants** — no adjustment; body text is readable on these lighter surfaces as-is.

This automatic adaptation works consistently in both the editor and on the front end.
...
Search results are handled by the standard WordPress search template, with a content header that displays the query.
...
```
v-dark-background              → Dark surface; text, links, headings, buttons,
                                  forms, and tables adapt automatically
v-gray-background              → Light gray surface (action tint)
```

The full editor palette also produces functional background classes. Text adapts automatically based on the lightness of the chosen background:

```
has-action-background-color    → Base brand color; text goes white
has-action-dark-background-color
has-action-darker-background-color
                               → Dark/darker variants; full dark context applied

has-accent-1-background-color  → Base accent-1; text goes white
has-accent-1-dark-background-color
has-accent-1-darker-background-color
                               → And so on for accent-2, accent-3
```

Light, lighter, and tint variants (`has-action-light-background-color`, etc.) receive no automatic text adjustment — body text is readable on those surfaces as-is.
